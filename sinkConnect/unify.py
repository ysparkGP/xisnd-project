...
def frequency_priority(li, column_name, condition_value):
    ...
    result_li = []
    for record in li:
        if getattr(record, column_name) == value:
            result_li.append(record)
    return result_li
    ...
    try:
        ...
        for logic in json['logic']:
            ...
            for unify in temp_unify_priorities:
                if getattr(unify, field) is not None:
...
def unify_start(session, seq, task_dml_tp, asis_uuid, tobe_uuid, effected_uuids, changed_effected_uuids):
    try:
        ...
        if task_dml_tp == 'INSERT':
            ...
            if asis_uuid == tobe_uuid: 
                ...
                if asis_last_link_unified_record : 
                    ...
                    if asis_last_unified_record: 
                        decide_priority(session, asis_last_unified_record, seq, asis_uuid)
            ...
            if last_link_record :
                # 남아있다면 통합 레코드 업데이트
                if unified_record:
                    decide_priority(session, unified_record, seq, asis_uuid)
            else :
        ...
        if changed_effected_uuids != None and len(changed_effected_uuids) > 0:
            ...
            for changed_effected_uuid in changed_effected_uuids:
                ...
                if changed_unified_record:
                    decide_priority(session, changed_unified_record, seq, changed_effected_uuid)
                    # 멀티쓰레딩으로 인한 동시성 문제때문에 INSERT -> UPSERT
